...
Invoke `needs-stories` to derive stories from the intent. 5 stories identified.

Propose feature grouping based on story cohesion:
...
Additionally:
  ...
  - needs-architecture: Document architecture (after implementation)

Risk: HIGH (new features, 3 feature packages, full lifecycle)
Estimated: 15+ artifacts + code
...
:capabilities-invoked: needs-stories, needs-spec, needs-design, needs-adr, needs-tasks, needs-implementation, needs-architecture
:constraints-checked: Security (pass), Architecture (pass), Quality (pass), Licensing (pass)
:result: Achieved
:artifacts-modified: 12 feature artifacts, 3 ADRs, 1 architecture doc, source code
....
