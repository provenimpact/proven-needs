...
2. Reconcile the design document -- review what was actually built against what the design describes, update any sections that diverged during implementation, and ensure `:status:` remains `Current`:
+
....
docs/features/shopping-cart/design.adoc -> :status: Current  (reconciled)
....
...
Set tasks status to Implemented. Reconcile design to reflect what was built."
....

Report to the orchestrator that the shopping-cart feature implementation is complete.
