...
This file shows a complete design document for the `shopping-cart` feature of an e-commerce project. The feature's Gherkin scenarios are used as input. The design includes Mermaid diagrams for component interactions and the primary user flow.

== Context

Given:

* `docs/features/shopping-cart/cart-management.feature` with scenarios @CART-001 through @CART-005 covering add-to-cart, duplicate handling, view cart, update quantity, and remove item
...
=== Feature: Cart Management (cart-management.feature)

Components:: ProductCard (existing), CartContext, CartIcon, Cart Page, POST/PATCH/DELETE /api/cart/items, CartService
Scenarios::
* @CART-001: Add product to cart -> ProductCard button triggers POST /api/cart/items via CartService, CartContext updates, CartIcon reflects count
* @CART-002: Adding duplicate product increments quantity -> CartService increments quantity if product already in cart
* @CART-003: View cart contents -> Cart Page reads CartContext and renders line items with prices and total
* @CART-004: Update item quantity -> PATCH /api/cart/items via CartService, CartContext updates, Cart Page re-renders with updated total
* @CART-005: Remove item from cart -> DELETE /api/cart/items via CartService, CartContext updates, Cart Page removes item and recalculates total
....
